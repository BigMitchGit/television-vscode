...
async function openFiles(paths, workspaceFolder) {
    await Promise.all(paths.map(async (filePath, _) => {
...
async function toggleFileFinderHandler() {
    ...
    if (!vscode.workspace.workspaceFolders) {
        vscode.window.showInformationMessage("[Television] - No workspace is open.");
        return;
    }
    const workspaceFolder = vscode.workspace.workspaceFolders[0].uri;
    // Try to find an existing terminal with the name "TV Finder"
    if (maybe_close_terminal(vscode.window.terminals.find(t => t.name === "TV Finder"))) {
        return;
    }
    ...
    const closeListener = vscode.window.onDidCloseTerminal(async (t) => {
        if (t === terminal) { // Ensure it's our terminal
...
async function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand('television.ToggleFileFinder', toggleFileFinderHandler);
...
function warn_binary_not_found() {
    vscode.window.showErrorMessage("The television binary doesn't seem to be available. Try installing it by following the installation docs.", "Installation docs").then((selection) => {
        if (selection === "Installation docs") {
            vscode.env.openExternal(vscode.Uri.parse("https://github.com/alexpasmantier/television/wiki/Installation"));
        }
    });
}
